...
/**********************************************************************/
void* accept_request(void* tclient)//接收客户端的连接，并读取请求数据
{
    char buf[1024];
    int numchars;
    char method[255];
    ...
    i = 0; j = 0;
    /*把客户端的请求方法存到 method 数组。对于HTTP报文来说，第一行的内容即为报文的起始行，
    格式为<method> <request-URL> <version>，每个字段用空白字符相连*/
...
void execute_cgi(int client, const char *path, const char *method, const char *query_string)
{
    ...
    if (pid == 0)  /* child: CGI script */
    {
        ...
        /*用 execl 运行 cgi 程序
        execl()用来执行参数path字符串所代表的文件路径，接下来的参数代表执行该文件时传递过去的argv(0)、argv[1]……，
        最后一个参数必须用空指针(NULL)作结束。
        返回值，如果执行成功则函数不会返回，执行失败则直接返回-1，失败原因存于errno中。*/
        if(execl(path, path, NULL) == -10)
	    {
	        error_die("execl");
	    }
        exit(0);
...
int main(void)
{
    ...
    /*初始化 httpd 服务，包括建立套接字，绑定端口，进行监听等,返回监听套接字*/
